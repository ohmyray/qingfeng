...
// getSpecJSON 获取文档 JSON（统一入口）
func getSpecJSON(cfg Config) []byte {
	// 优先级：DocJSON > DocPath > 自动生成
	if cfg.DocJSON != nil {
		return cfg.DocJSON
	}
	...
	if cfg.DocPath != "" {
		...
		log.Printf("[QingFeng] 读取文档文件失败: %v\n", err)
	}

	// 自动生成
	if cfg.AutoGenerate {
		data, err := generateSpec(cfg)
		if err != nil {
			log.Printf("[QingFeng] 自动生成文档失败: %v\n", err)
			return nil
		}
		return data
	}
